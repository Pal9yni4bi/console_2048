...
bool game::checkAndCollapse(unsigned short *from, unsigned short *to, bool *isSmthChanged) {
	if (*from == *to) {
		...
		*from = 0; // убираем значение из ячейки-донора
		...
		return true; // предотвращаем повторное коллапсирование текущей ячейки
	}
	...
	for (i = 0; i < FIELD_SIZE; i++) {
		for (y = 0; y < FIELD_SIZE; y++) {
			...
			case 2048:
				...
				puts("2048");
				break;
			default: // будет отрабатывать, когда ячейка пуста
				puts("    ");
				break;
			}
		}
...
void game::spawn() {
	...
	// ... и помещаем в неё число 2 или 4 с вероятностью 2 к 8
